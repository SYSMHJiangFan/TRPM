    ...
    for milnet in milnet_list:
        ...
        with torch.no_grad():
            for i, item in enumerate(test_df):
                bag_feats  = load_stacked_data(item, data_info[item], args.input_feats_path)
...
def main():
    ...
    test_predictions_list = infer(args, all_path_no_list, milnet, thresholds=args.thresholds)

    print(test_predictions_list.shape)

    output_content_dict = {'path_no': all_path_no_list}
