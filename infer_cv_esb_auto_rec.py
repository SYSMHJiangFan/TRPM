...
def infer(args, test_df, milnet_list, thresholds=None, combine=True):

    test_pred_list = []
    data_info = load_pkl(args.input_label_info_dict)

    for milnet in milnet_list:
        test_labels = []
        test_predictions = []
    ...
    if combine:
        ...
        test_avg_predictions = np.mean(test_pred_arr, axis=0)
        auc_value, _, best_thresholds_optimal = multi_label_roc(test_labels, test_avg_predictions, args.num_classes, pos_label=1)
        thresholds_optimal = best_thresholds_optimal
        class_prediction_bag = copy.deepcopy(test_avg_predictions)
        class_prediction_bag[test_avg_predictions>=thresholds_optimal[0]] = 1
        class_prediction_bag[test_avg_predictions<thresholds_optimal[0]] = 0
        test_cls_predictions = class_prediction_bag
        print(classification_report(test_labels, test_cls_predictions, digits=4))
        print(confusion_matrix(test_labels, test_cls_predictions))
        test_pred_list.append(test_avg_predictions)

    test_pred_list = np.array(test_pred_list)

    return  test_pred_list, test_labels
...
def main():
    ...
    parser.add_argument('--input_model_exp_dir', default=None, type=str, help='input_label_info_dict')
    parser.add_argument('--output_dir', default=None, type=str, help='output_dir')
    parser.add_argument('--postfix', default=None, type=str, help='postfix')
    ...
    def init_model(args, model_path_list):
        model_list = []
    ...
    # init models
    all_exp_dirs = os.listdir(args.input_model_exp_dir)
    model_rec_path_list = [os.path.join(args.input_model_exp_dir, exp_dir, 'fold_best_result_rec.csv') for exp_dir in all_exp_dirs]
    print(len(model_rec_path_list))

    valid_model_rec_path_list = [tmp_path for tmp_path in model_rec_path_list if os.path.isfile(tmp_path)]
    print('### valid model rec num:')
    print(len(valid_model_rec_path_list))
    print('### valid model rec path list: {} ####'.format(valid_model_rec_path_list))
    for current_model_rec_path in valid_model_rec_path_list:
        print('#### current model rec path: {} ####'.format(current_model_rec_path))
        sub_dirname = os.path.basename(os.path.dirname(current_model_rec_path))
        model_path_list = pd.read_csv(current_model_rec_path)['best_model'].tolist()

        milnet = init_model(args, model_path_list)


            
        test_predictions_list, test_labels = infer(args, all_path_no_list, milnet, thresholds=args.thresholds)

        print(test_predictions_list.shape, test_labels.shape)

        output_content_dict = {'path_no': all_path_no_list, 'label': test_labels}

        for i in range(len(test_predictions_list)-1):
            output_content_dict['pred_model_{}'.format(i)] = test_predictions_list[i]

        output_content_dict['pred_model_avg'] = test_predictions_list[-1]


        current_save_path = os.path.join(save_path, sub_dirname + '_' + '{}.csv'.format(args.postfix))
        os.makedirs(os.path.dirname(current_save_path), exist_ok=True)
        pd.DataFrame(output_content_dict).to_csv(current_save_path, index=False)
